...
import { approveStudentJoinClass, rejectStudentJoinClass, getClassesByTeacherId } from '@/services/teacher/classes'
...
export function PendingRequestsModal({
    ...
    const fetchPendingRequests = useCallback(async () => {
        if (!isOpen || !classItem?.id || !classItem.teacherId || !accessToken) {
            return
        }
        ...
        try {
            const response = await getClassesByTeacherId(accessToken, classItem.teacherId, true)
            const targetClass = response.classes.find((item) => item.id === classItem.id)

            setPendingRequests(
                targetClass?.pendingStudentsList ??
                targetClass?.classMembers?.filter((request) => !request.isApproved && !request.isRejected && !request.isBanned) ??
                [],
            )
        } catch (error) {
    ...
    }, [accessToken, classItem?.id, classItem?.teacherId, isOpen, language])
    ...
    const removePendingRequest = (studentId: string) => {
        setPendingRequests((currentRequests) => currentRequests.filter((request) => request.studentId !== studentId))
    }
    ...
    return (
        <ModalWrapper
            ...
            contentClassName="w-[40vw] sm:max-w-4xl"
            footer={null}
        >
